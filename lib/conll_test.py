...
def final_1best_unlabel(length, derivations, parse_probs, rel_probs):
    '''
    output 1best set of edges from derivation chart
    '''
    print('final_derivation')
    final_kbests = derivations[(0,0,length+1)]
    print(final_kbests)
    
    for j,kbest in enumerate(final_kbests):
        print(kbest)
        print(kbest.depedges)
    best_hyp = final_kbests[0]
    #print('best_hyp')
    #print(best_hyp)

    edges = []
    for hd,md in best_hyp.depedges:
        prb = parse_probs[md,hd]
        assert prb > 0.0
        edges.append((md,hd,prb))#prb
    best_tree = sorted(edges, key=lambda x: x[0]) 
    pprint(edges)
    return best_tree
...
def to_conllu_unlabel(out_path, best_tree, sent, tags):
    ...
    keys = list(sorted(temp_d.keys()))
    conll_lines = ['\t'.join([str(tail),sent[tail-1],sent[tail-1].lower(),tags[tail-1],'_','_','_','_',str(temp_d[tail]),'_']) for tail in keys]
...
def pruned_loader(pruned_dir):

    all_forests = []

    for n in range(1,2):
        forest_dir=os.path.join(pruned_dir,str(n))
        pruned_paths = glob.glob(os.path.join(forest_dir,'*.pruned'))
        l = len(pruned_paths)

        ordered_pruned_paths = [os.path.join(forest_dir,str(i+1)+'.pruned') for i in range(l)]
        #print(ordered_pruned_paths)

        forests = []
        for pruned_path in ordered_pruned_paths:
...
def pkl_loader(pkl_dir):
    ...
    for n in range(1,2): ## id of buckets in parser
        ...
        avg_len=sum([len(sent) for sent in sents])/len(sents)
...
def pkl_analysis(pkl_dir):
    ...
    for n in range(1,11): ## id of buckets in parser
        ...
        total_he=0
        total_nodes=0
        total_he_per_nodes=0
        for forest in tqdm(forests):
            he = len(forest['hyperedges'])
            total_he+=he
            nodes, he_per_nodes = count_nodes(forest)
            total_nodes+=nodes
            total_he_per_nodes+=he_per_nodes
            
            #print(len(forest['hyperedges']))
        print('HE')
        print(total_he)
        print(total_he/len(sents))
        all_total_he+=total_he
        print('NODES')
        print(total_nodes)
        print(total_nodes/len(sents))
        all_total_nodes+=total_nodes
        print('HE PER NODES')
        print(total_he_per_nodes)
        print(total_he/total_nodes)
        all_total_he_per_nodes+=total_he/total_nodes
        
        ## deprel

    print('Total: '+str(len(all_sents)))
    print('HE: '+str(all_total_he/len(all_sents)))
    print('Nodes: '+str(all_total_nodes/len(all_sents)))
    print('Density: '+str(all_total_he_per_nodes/10))


    

    return all_forests, all_sents
...
def count_nodes(forest):
    ...
    for he in he_ids:
        sp = he.split('_')
        Xspan = (sp[0],sp[3],sp[5])
        Xspans.add(Xspan)
    
    total_Xspan = len(Xspans)
    ...
    return total_Xspan, total_he/total_Xspan
