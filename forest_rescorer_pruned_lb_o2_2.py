...
def cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, vdl, nvdl, rescore_config, sent, tags, deprel_path, out_path, K, cnt, debug=False):
    ...
    for X_id in topological_id:
        ...
        if Xspan in terminals:
            ...
            continue
        kbest(X_id, Xspan, derivations, terminals, node2edges, id2Xspan, Xspan2id, topological_id, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, vdl, nvdl, sent, tags, rescore_config, K, cnt)
...
def kbest(X_id, Xspan, derivations, terminals, node2edges, id2Xspan, Xspan2id, topological_id, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, vdl, nvdl, sent, tags, rescore_config, K, cnt):
    ...
    for incoming_edge in incoming_edges:
        ...
        pushsucc(heap, derivations, terminals, parse_probs, vdl, nvdl, sent, tags, 0, 0, unit, visited)
    ...
    while len(heap)>0:
        ...
        ## next grid
        pushsucc(heap, derivations, terminals, parse_probs, vdl, nvdl, sent, tags, ka+1, kb, unit, visited)
        pushsucc(heap, derivations, terminals, parse_probs, vdl, nvdl, sent, tags, ka, kb+1, unit, visited)
...
def find_child(edges,md,rooted_sent):
    ## edges = {(h,m),...}
    child_ids = []
    for edge in edges:
        if edge[0] == md:
            child_ids.append(edge[1])

    child_ids = sorted(child_ids)
    children = [rooted_sent[m] for m in child_ids]
    pat = '{0} ( {1} )'.format(rooted_sent[md],' '.join(children))

    return pat

def pushsucc(heap, derivations, terminals, parse_probs, vdl, nvdl, sent, tags, ka, kb, unit, visited):
    '''
    ...
    rooted_tags = ['ROOT']+tags

    v_pat2nodes,v_hd2pat=vdl
    nv_pat2nodes,nv_hd2pat=nvdl
    v_pat_l = v_hd2pat[hd]
    nv_pat_l = nv_hd2pat[hd]
    ...
    if Aspan in terminals and Bspan in terminals:
        ...
        edges = set([edge[:2] for edge in rhs.depedges])
        pat = find_child(edges,md,rooted_sent)
        ...
        edges = set([edge[:2] for edge in lhs.depedges])
        pat = find_child(edges,md,rooted_sent)
        ...
        edges = set([edge[:2] for edge in lhs.depedges])|set([edge[:2] for edge in rhs.depedges])
        pat = find_child(edges,md,rooted_sent)
    ...
    if rescore_config['RESCORE']=='True':
        if hd*md!=0:
            newlogp = rescore(logp,md,hd,pat,v_pat_l,nv_pat_l,rooted_tags,rescore_matrix_V,rescore_matrix_NV,vdl,nvdl,rescore_config)
        else:
            newlogp = logp
    else: #for debug
        newlogp = logp
...
def rescore(logp,md,hd,pat,v_pat_l,nv_pat_l,tags,rescore_matrix_V,rescore_matrix_NV,vdl,nvdl,rescore_config):
    ...
    logger.debug('pat: '+str(pat))
    logger.debug('md hd: '+str(md)+' '+str(hd))
    if rescore_mode=='A':
        #if rescore_matrix_V[md] is not None: # parent node is verb
        if tags[md]=='VERB':
            logger.debug('matV: '+str(rescore_matrix_V[md].keys()))
            if pat in rescore_matrix_V[md].keys():
                bert_score = np.log(rescore_matrix_V[md][pat][hd])
                logger.debug('bertscore: '+str(bert_score))
                return logp + beta + alpha*bert_score
        else:
            logger.debug('matNV: '+str(rescore_matrix_NV[md].keys()))
            if pat in rescore_matrix_NV[md].keys():
                bert_score = np.log(rescore_matrix_NV[md][pat][hd])
                logger.debug('bertscore: '+str(bert_score))
                return logp + beta + alpha*bert_score
        return logp
        ...
        if tags[md]=='VERB': # parent node is verb
            #bert_score = np.log(rescore_matrix_V[md][hd])
            if pat in v_pat_l:
                bert_score = np.log(rescore_matrix_V[md][pat])
                return logp + beta + alpha*bert_score
        return logp
        ...
        if tags[md]!='VERB': # parent node is verb
            if pat in nv_pat_l:
                bert_score = np.log(rescore_matrix_NV[md][pat])
                return logp + beta + alpha*bert_score
        return logp
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vdl_path', type=str, help='dict pkl')
    parser.add_argument('--nvdl_path', type=str, help='dict pkl')
    ...
    if args.rescore=='True':
        ...
        if args.from_saved=='True':
            ...
            with open(args.vdl_path, 'rb') as pd:
                vdl_saved = pkl.load(pd)
            with open(args.nvdl_path, 'rb') as pd:
                nvdl_saved = pkl.load(pd)
            ...
            for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                ...
                cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, vdl, nvdl, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt)
