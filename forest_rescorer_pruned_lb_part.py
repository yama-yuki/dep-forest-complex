...
from lib.conll import final_1best_label, to_conllu_label, pkl_n_loader, rels_loader, pruned_n_loader
...
def kbest(X_id, Xspan, derivations, terminals, node2edges, id2Xspan, Xspan2id, topological_id, tags, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, K, cnt):
    ...
    for incoming_edge in incoming_edges:
        ...
        pushsucc(heap, derivations, terminals, tags, parse_probs, 0, 0, unit, visited)
    ...
    while len(heap)>0:
        ...
        ## next grid
        pushsucc(heap, derivations, terminals, tags, parse_probs, ka+1, kb, unit, visited)
        pushsucc(heap, derivations, terminals, tags, parse_probs, ka, kb+1, unit, visited)
...
def pushsucc(heap, derivations, terminals, tags, parse_probs, ka, kb, unit, visited):
    '''
    ...
    if rescore_config['RESCORE']=='True':
        if hd*md!=0:
            newlogp = rescore(logp,md,hd,tags,rescore_matrix_V,rescore_matrix_NV,rescore_config)
        else:
            newlogp = logp
    else: #for debug
        newlogp = logp
...
def rescore(logp,md,hd,tags,rescore_matrix_V,rescore_matrix_NV,rescore_config):
    ...
    if rescore_mode=='A':
        if tags[md][:1]=='V': #tags[md]=='VERB'# parent node is verb
            try:
        ...
        if tags[md][:1]=='V': # parent node is verb
            try:
                bert_score = np.log(rescore_matrix_V[md-1][hd-1])
                return logp + beta + alpha*bert_score
            except:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rescore_mode', type=str, help='V:verbial,N:non-verbial,A:both,All:all')
    ...
    if args.save_rescore=='True':
        logger.info('0: save_rescore')
        logger.info('1: Loading Model Configurations')
        
        ##-------------------------------------------------------------------------##
        ## LOAD DATA

        logger.debug('2: Checking Existing Files')
        if os.path.exists(args.out_path):
            sys.exit('File: '+args.out_path+' Exists. Already Searched with these Parameters.')

        logger.debug('3: Loading Parsed Forests')

        rescore_matrices = []
        for n in range(1,11):
            _,all_parse_probs,all_rel_probs,all_sents,all_tags = pkl_n_loader(pkl_dir,n)
            all_forests = pruned_n_loader(pruned_dir,n)
            rel_vocabs = rels_loader(args.rel_vocab_path)
            logger.debug('Done Loading File: '+str(n))
            ...
            if args.rescore_mode=='V':
                ...
                logger.info('5: Head Prediction Done: '+str(n))
                rescore_matrices.extend(rescore_matrix_V)     

                save_path = args.v_path
                ...
                logger.info('5: Head Prediction Done: '+str(n))
                rescore_matrices.extend(rescore_matrix_NV)     
  
                save_path = args.nv_path
                ...
                logger.info('5: Head Prediction Done: '+str(n))
                rescore_matrices.extend(rescore_matrix_All)     

                save_path = args.all_path
            
            else:
                sys.exit('Specify rescore_mode')

        logger.info('6: Saving Head Predictions')
        with open(save_path, 'wb') as p:
            pkl.dump(rescore_matrices, p)
    ...
    for n in range(1,11):
            ...
            if args.from_saved=='True':
                ...
                if args.rescore_mode=='A':
                    ...
                    for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                        rescore_matrix_V = rescore_matrix_V_saved[rescore_cnt]
                        rescore_matrix_NV = rescore_matrix_NV_saved[rescore_cnt]
                    ...
                    logger.debug('4: Do Cube Pruning: '+str(n))
                    for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                        rescore_matrix_V = rescore_matrix_V_saved[rescore_cnt]
                        ...
                        cnt+=1
                        rescore_cnt+=1
                    logger.debug('5: Done Cube Pruning: '+str(n))
                    ...
                    for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                        ...
                        rescore_matrix_NV = rescore_matrix_NV_saved[rescore_cnti]
                        cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt)
                        cnt+=1
                        rescore_cnt+=1
                    logger.debug('5: Done Cube Pruning: '+str(n))     
                    ...
                    logger.debug('4: Do Cube Pruning')
                    for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                        rescore_matrix_V = rescore_matrix_All_saved[rescore_cnt]
                        rescore_matrix_NV = None
                        ##node, edge, ratio, tokens = 
                        cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt)
                        ##(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.out_path, K)
        ...
        ## CUBE PRUNING without BERT
            ...
            logger.debug('2: vanilla: '+str(n))
            logger.debug('3: Do Cube Pruning: '+str(n))

            for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                rescore_matrix_V = None
                rescore_matrix_NV = None
                ##node, edge, ratio, tokens = 
                cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, 0)
                ##(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.out_path, K)
                line_cnt+=1
                logger.debug('4: Done: '+str(n))
                '''
                n+=node
                e+=edge
                p+=ratio
    ...
    logger.debug('ratio: '+str(p))
    logger.debug('tok: '+str(t))
    logger.debug('cnt: '+str(line_cnt))
    '''
        ##-------------------------------------------------------------------------##
