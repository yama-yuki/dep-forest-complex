...
def cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, vdl, nvdl, rescore_config, sent, tags, deprel_path, out_path, K, cnt, debug):
...
def find_child(edges,hd,rooted_sent):
    ## edges = {(h,m),...}
    child_ids = []
    for edge in edges:
        if edge[0] == hi:
    ...
    pat = '{0} ( {1} )'.format(rooted_sent[hi],' '.join(children))
...
def pushsucc(heap, derivations, terminals, parse_probs, vdl, nvdl, sent, tags, ka, kb, unit, visited):
    ...
    '''
    '''
    [3.1] load 2nd-order pattern dict
    '''
    ...
    '''
    [3.2] compute score for new Xspan
    '''
    if Aspan in terminals and Bspan in terminals:
        ...
        pat = '{0} ( {1} )'.format(rooted_sent[hd], rooted_sent[md])
        ...
        pat = find_child(edges,hd,rooted_sent)
...
def rescore(logp,md,hd,pat,v_pat_l,nv_pat_l,tags,rescore_matrix_V,rescore_matrix_NV,vdl,nvdl,rescore_config):
    ...
    if rescore_mode=='A':
        ...
        if tags[md]=='VERB':
            if pat in v_pat_l:
                #logger.debug('rescorematrix: '+str(rescore_matrix_V))
...
if __name__ == '__main__':
    ...
    if args.rescore=='True':
        ...
        if args.from_saved=='True':
            ...
            ## dl = (pattern2nodes,hi2pat) #dict
            ## pat2nodes={'ROOT ( No )': [0, 1], 'ROOT ( was )': [0, 4], '. ( was )': [8, 4], ...}
            ## hi2pat={0: ['ROOT ( No )', 'ROOT ( was )', 'ROOT ( , )', 'ROOT ( it )'], 8: ['. ( was )',}
            ...
            for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                ...
                cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, vdl, nvdl, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt, debug=False)
    ...
    ## CUBE PRUNING without BERT
        ...
        for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
            ...
            ##node, edge, ratio, tokens = 
            cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt, debug=True)
            ##(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.out_path, K)
            cnt+=1
            logger.debug('4: Done')
            '''
