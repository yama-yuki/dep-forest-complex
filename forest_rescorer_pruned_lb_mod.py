...
from rescore_module.my_lib.rescore_window import LocalDependencyModel, LocalContextDataset, make_data, load_model, predict
...
def cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, deprel_path, out_path, K, cnt, debug=False):
    ...
    for X_id in topological_id:
        ...
        if Xspan in terminals:
            ...
            continue
        kbest(X_id, Xspan, derivations, terminals, node2edges, id2Xspan, Xspan2id, topological_id, tags, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, K, cnt)
    ...
    ## write out labeled conllu
    to_conllu_label(out_path, best_tree, sent, tags, deprel_path)

def kbest(X_id, Xspan, derivations, terminals, node2edges, id2Xspan, Xspan2id, topological_id, tags, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, K, cnt):
    ...
    for incoming_edge in incoming_edges:
        ...
        pushsucc(heap, derivations, terminals, tags, parse_probs, 0, 0, unit, visited, sent)
    ...
    while len(heap)>0:
        ...
        ## next grid
        pushsucc(heap, derivations, terminals, tags, parse_probs, ka+1, kb, unit, visited, sent)
        pushsucc(heap, derivations, terminals, tags, parse_probs, ka, kb+1, unit, visited, sent)
...
def pushsucc(heap, derivations, terminals, tags, parse_probs, ka, kb, unit, visited, sent):
    '''
    ...
    if rescore_config['RESCORE']=='True':
        if hd*md!=0:
            newlogp = rescore(logp,md,hd,tags,rescore_matrix_V,rescore_matrix_NV,rescore_config, sent)
        else:
            newlogp = logp
    else: #for debug
        newlogp = logp
    ...
    if not items or not newlogp.any():
        return

    heapq.heappush(heap, [-newlogp, items])

def rescore(logp,md,hd,tags,rescore_matrix_V,rescore_matrix_NV,rescore_config,sent):
    ...
    if rescore_mode=='A':
        if tags[md]=='VERB': # parent node is verb
            try:
        ...
        if tags[md]=='VERB': # parent node is verb
            ...
            try:
                ...
                return logp
    elif rescore_mode=='All':
        bert_score = np.log(rescore_matrix_V[md-1][hd-1])
        return logp + beta + alpha*bert_score
    elif rescore_mode=='W':
        context2softmax = rescore_matrix_V

        vocab = tokenizer.get_vocab()
        d={}
        for key, value in vocab.items():
            d[key] = int(value)
        sent_id = [d[w] if w in d else 0 for w in sent]

        if tags[md] == 'VERB':
            child = sent[md]
            parent_cand_vocab_id = sent_id[hd]

            context = create_context(sent, md, rescore_config['window_size'])

            try:
                softmax = context2softmax[context]
                bert_score = np.log(softmax[parent_cand_vocab_id])

                new_score = logp + beta + alpha*bert_score
                if new_score == score:
        ...
        if tags[md]=='VERB':
            child = sent[md]
            parent_cand_vocab_id = sent_id[hd]
            if md!=0 and md!=len(sent)-1 :
                context = f"[CLS] {sent[md-1]} {child} {sent[md+1]} [SEP]"
            elif md==0:
                context = f"[CLS] {child} {sent[md+1]} [SEP]"
            else:
                context = f"[CLS] {sent[md-1]} {child} [SEP]"
            try:
                softmax = context2softmax[context]
                bert_score = np.log(softmax[parent_cand_vocab_id])
...
def create_context(sent, md, window_size):
    start_index = max(md - window_size, 0)
    end_index = min(md + window_size + 1, len(sent))
    context = " ".join(sent[start_index:end_index])
    return f"[CLS] {context} [SEP]"

def make_eval_data(all_sents, all_tags):
    data = {"data": [{"paragraphs": 
                        [{"context": "model developed by",
                            "qas": [
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rescore_all_cfg', type=str, help='file path for rescore config (.cfg)')
    ...
    parser.add_argument('--all_path', type=str, help='allmodel pkl')

    parser.add_argument('--rel_vocab_path', type=str, help='path to rels.txt')
    parser.add_argument('--deprel_path', type=str, default='/home/is/yuki-yama/work/d3/dep-forest-complex/biaffine_forest/pkl/deprel.pkl')
    parser.add_argument('--rescore_mode', type=str, help='V:verbial,N:non-verbial,A:both,All:all,W:local context with window_size')

    parser.add_argument('--window_size', type=int, default=1, help='window_size for local context model')
    ...
    if args.save_rescore=='True':
        ...
        elif args.rescore_mode=='All':
            if not os.path.exists(args.rescore_all_cfg):
                sys.exit('rescore_all.cfg Not Found') 
            cfg_all = configparser.RawConfigParser(interpolation=configparser.ExtendedInterpolation())
            cfg_all.read(args.rescore_all_cfg)
            remodel_All = RescoreModel(cfg_all)

            logger.info('4: Predicting All')
            rescore_matrix_All = [remodel_All.head_prediction_NV(sent, tags) for sent,tags in tqdm(zip(all_sents,all_tags))]
            with open(args.all_path, 'wb') as p:
                pkl.dump(rescore_matrix_All, p)     
            rescore_matrix_All = []

        elif args.rescore_mode=='W':
            if not os.path.exists(rescore_v_cfg):
                sys.exit('rescore_v_window1.cfg Not Found')        
            cfg_v = configparser.RawConfigParser(interpolation=configparser.ExtendedInterpolation())
            cfg_v.read(rescore_v_cfg)

            out_path = args.v_path
            #eval_data_path = args.data_path
            model_save_path = cfg_v['OS']['model_path']
            logger.info(all_sents[:1])##[['No', ',', 'it', 'was', "n't", 'Black', 'Monday', '.']]
            logger.info(all_tags[:1])##[['ADV', 'PUNCT', 'PRON', 'VERB', 'PART', 'PROPN', 'PROPN', 'PUNCT']]
            
            remodel_V = LocalDependencyModel()
            load_model(remodel_V,model_save_path)
            logger.info('hoge')

            eval_dataloader = make_data(all_sents,all_tags,args.window_size)
            logger.info(len(eval_dataloader))

            logger.info('4: Predicting V')
            #rescore_matrix_V = [remodel_V.head_prediction_V(sent, tags) for sent,tags in tqdm(zip(all_sents,all_tags))]
            softmax_output = predict(remodel_V,eval_dataloader)
            logger.info('hoge')
            with open(args.v_path, 'wb') as pv:
                pkl.dump(softmax_output, pv)        
            #rescore_matrix_V = []

        logger.info('5: Head Prediction Done & Saved')
        sys.exit()
        ...
        if args.from_saved=='True':
            ...
            if args.rescore_mode=='A':
                ...
                for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                    ...
                    cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt)
                    ##(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.out_path, K)
                    cnt+=1
            
            elif args.rescore_mode=='V':
                with open(args.v_path, 'rb') as pv:
                    rescore_matrix_V_saved = pkl.load(pv)
                ...
                with open(args.all_path, 'rb') as p:
                    rescore_matrix_All_saved = pkl.load(p)

                logger.debug('4: Do Cube Pruning')
                for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
                    rescore_matrix_V = rescore_matrix_All_saved[i]
                    rescore_matrix_NV = None
                    ##node, edge, ratio, tokens = 
                    cube(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.deprel_path, args.out_path, args.K, cnt)
                    ##(forest, parse_probs, rel_probs, rescore_matrix_V, rescore_matrix_NV, rescore_config, sent, tags, args.out_path, K)
    ...
    ## CUBE PRUNING without BERT
        ...
        for i,(forest,parse_probs,rel_probs,sent,tags) in enumerate(tqdm(zip(all_forests,all_parse_probs,all_rel_probs,all_sents,all_tags))):
            ...
            '''
        '''
        logger.debug('4: '+str(n))
        logger.debug('4: '+str(e))
        logger.debug('4: '+str(p))
        logger.debug('4: '+str(t))
        '''
    ##-------------------------------------------------------------------------##
